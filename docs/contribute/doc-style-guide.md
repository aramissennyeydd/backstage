...
### Use `create-app` as the documentation baseline

Across all Backstage documentation, assume that the reader starts with a fresh
app generated by `create-app`, unless the document explicitly defines a
different starting point. Treat every package, plugin, and configuration
provided by that template as already present.

Do not ask readers to install, enable, register, or confirm template defaults.
Before adding an installation instruction, check the template in
`packages/create-app/templates/default-app`. Document installation only when
the target workspace does not already declare the required package. Document
configuration only when the reader must change a value supplied by the
template.

| Do                                                                                    | Don't                                                                                                    |
| :------------------------------------------------------------------------------------ | :------------------------------------------------------------------------------------------------------- |
| Enable Dynamic Client Registration in `app-config.yaml`.                              | Confirm that the auth frontend and backend are installed before changing the registration configuration. |
| Add a required dependency to a newly created plugin that does not already declare it. | Install the Search backend in `packages/backend` when the `create-app` template already includes it.     |
